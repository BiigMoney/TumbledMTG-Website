          ...
          request.post(options, function(error, response, body){
            if (!error && response.statusCode === 200) {
                ...
                request.get(dataOptions, function(error, response, body){
                    ...
                    if (!error && response.statusCode === 200) {
                        db.collection('users').where('id', "==", response.body.id).get().then(snap => {
                            if(snap.size === 0){
...
exports.getReplays = (req,res) => {
	db.collection('replays').orderBy('id','desc').limit(500).get().then(data => {
		let replayList = []
		data.forEach(doc => {
...
exports.getDecklists = (req,res) => {
	db.collection('decklists').orderBy('createdAt','desc').get().then((data) => {
		let decklists = [];
		data.forEach((doc) =>{
			let lol = {
				decklistId: doc.id,
...
exports.getCards = (req,res) => {
    	admin.database().ref().child('cards').get().then(snap => {
...
exports.getDecklist = (req,res) => {
	let decklistData = {};
	db.doc(`/decklists/${req.params.decklistId}`).get().then(doc =>{
		if(!doc.exists){
...
exports.decklistsByUser = (req,res) => {
	db.collection('decklists').where("uploadId", "==", req.params.userId).orderBy("createdAt", "desc").get().then(snap => {
		let decklists = []
		snap.forEach(doc => {
			let lol = {
				decklistId: doc.id,
...
exports.deleteDecklist = (req,res) => {
    ...
    db.doc(`/decklists/${req.params.decklistid}`).delete().then(() => {
...
exports.addStars = (req,res) => {
    ...
    db.doc(`/decklists/${req.params.decklistId}`).get().then(doc =>{
        ...
        db.doc(`/decklists/${req.params.decklistId}`).update({
            stars: decklistStars
        }).then(() => {
...
exports.uploadReplay = (req,res) => {
    ...
    admin.storage().bucket("tumbledmtg-website").file(`replay_${req.body.id}.cor`).save(replay, {
        resumable: false,
        metadata: {
            metadata: {
        ...
        db.collection("replays").add(replayData).then(() => {
